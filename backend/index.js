...
// Basic validation for the connection string to provide clearer errors early
if (!mongoUri || !(mongoUri.startsWith('mongodb://') || mongoUri.startsWith('mongodb+srv://'))) {
  console.error('MongoDB connection error: MONGODB_URI is missing or malformed.');
  console.error('Please set MONGODB_URI in backend/.env and ensure it starts with "mongodb://" or "mongodb+srv://"');
  console.error('Example: mongodb+srv://<user>:<password>@cluster0.example.mongodb.net/yourDB?retryWrites=true&w=majority');
  // Exit so the developer notices and doesn't continue with a broken DB connection
  process.exit(1);
}
...
app.post('/api/visitor/in', authMiddleware, async (req, res) => {
  ...
  // Previous behavior: simply record IN and return success (no email)
...
app.post('/api/visitor/out', authMiddleware, async (req, res) => {
  ...
  try {
    ...
    // Previous behavior: just record OUT and return success (no email)
